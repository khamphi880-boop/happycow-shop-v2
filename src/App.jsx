...
import { ShoppingCart, Plus, Trash2, ChevronLeft, X, Upload, ClipboardList, Coffee, Zap, MapPin, Settings, Copy, CheckCircle, AlertCircle, LogIn, Eye } from 'lucide-react';
...
export default function App() {
  ...
  const [selectedSlip, setSelectedSlip] = useState(null); // สำหรับแสดงรูปสลิปขยายใหญ่
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      alert("สั่งซื้อสำเร็จ! บิลส่งเข้าแชทแล้วครับ 🐮");
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#F5EEDC] flex flex-col font-sans text-[#3D2C1E]">
      ...
      <main className="flex-1 pb-10">
          ...
          <div className="animate-in fade-in">
            ...
            <div className="p-5 grid grid-cols-2 gap-5">
              {menuItems.filter(i => i.category === activeCategory).map(item => (
                <div key={item.id} onClick={() => { setOptionModalItem(item); setTempOptions({sweetness: '100%', isBlended: false}); }} className="bg-white rounded-[2rem] overflow-hidden shadow-sm active:scale-95 transition-all">
        ...
        {view === 'cart' && (
          <div className="p-6 space-y-6 bg-white rounded-t-[3rem] mt-4 min-h-[85vh] shadow-2xl">
            ...
            {cart.length > 0 && (
              <div className="space-y-6 pt-6 border-t border-gray-100 text-center">
                <textarea value={address} onChange={e => setAddress(e.target.value)} placeholder="ที่อยู่จัดส่ง / เบอร์โทร..." className="w-full p-5 rounded-3xl bg-gray-50 h-32 text-sm outline-none" />
                <div className="bg-gray-50 p-6 rounded-[2.5rem] border-2 border-dashed border-gray-200">
                  ...
                  {storeSettings.qrCodeImage ? <img src={storeSettings.qrCodeImage} className="w-40 h-40 mx-auto mb-4 bg-white p-2 rounded-xl object-contain" /> : <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=PROMPTPAY:${storeSettings.promptPayNo}:${cart.reduce((s,i)=>s+(i.price*i.qty),0)}`} className="w-40 h-40 mx-auto mb-4 bg-white p-2 rounded-xl" />}
                  <button onClick={copyPromptPay} className="mb-6 bg-white border px-3 py-1.5 rounded-full text-[10px] font-bold inline-flex items-center gap-2">
                    {isCopied ? <CheckCircle size={14} className="text-green-500"/> : <Copy size={14}/>} {storeSettings.promptPayNo}
                  </button><br/>
                  <label className="cursor-pointer bg-[#3D2C1E] text-white py-4 px-8 rounded-2xl text-[11px] font-bold inline-flex items-center gap-2">
                  ...
                  {slipImage && <img src={slipImage} className="mt-4 h-32 mx-auto rounded-lg shadow-md" />}
                </div>
                <button onClick={handleOrder} disabled={isLoading || !slipImage} className={`w-full py-5 rounded-[2.5rem] font-bold text-lg ${slipImage ? 'bg-[#A67C52] text-white' : 'bg-gray-100 text-gray-300'}`}>{isLoading ? 'กำลังประมวลผล...' : `ยืนยันการสั่งซื้อ • ฿${cart.reduce((s,i)=>s+(i.price*i.qty),0)}`}</button>
              </div>
            )}
          </div>
        )}
          ...
          <div className="p-6 bg-white min-h-screen">
            ...
            <div className="flex gap-2 bg-gray-50 p-1 rounded-2xl mb-6">
              {['orders', 'menus', 'settings'].map(t => (
                <button key={t} onClick={() => setAdminTab(t)} className={`flex-1 py-3 rounded-xl text-xs font-bold ${adminTab === t ? 'bg-[#3D2C1E] text-white shadow-md' : 'text-gray-500'}`}>{t === 'orders' ? 'ออร์เดอร์' : t === 'menus' ? 'เมนู' : 'ตั้งค่า'}</button>
              ))}
            </div>
              ...
              <div className="space-y-4">
                {orders.map(o => (
                  <div key={o.id} className="border border-gray-100 p-4 rounded-2xl shadow-sm">
                    <div className="flex justify-between mb-2"><span className="font-bold text-sm">{o.lineName}</span><span className="text-orange-600 font-bold">฿{o.total}</span></div>
                    <div className="text-xs text-gray-500 mb-2">ที่อยู่: {o.address}</div>
                    <div className="space-y-1 border-t pt-2 mb-2">
                      {(o.items || []).map((i, idx) => <div key={idx} className="text-xs text-gray-600">{i.qty}x {i.name} ({i.isBlended?'ปั่น':'เย็น'})</div>)}
                    </div>
                    <div className="flex gap-2">
                      {/* แก้ไขปุ่มดูสลิป ให้เรียกฟังก์ชัน setSelectedSlip แทน window.open */}
                      <button onClick={() => setSelectedSlip(o.slipImage)} className="flex-1 bg-blue-50 text-blue-600 py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2"><Eye size={14}/> ดูสลิปโอนเงิน</button>
                      <button onClick={() => deleteDoc(doc(db, 'orders', o.id))} className="bg-red-50 text-red-500 px-4 rounded-xl"><Trash2 size={16}/></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {adminTab === 'menus' && (
              <div className="space-y-8">
                <div className="bg-gray-50 p-5 rounded-3xl border-2 border-dashed border-gray-200 space-y-4 text-center">
                  <h3 className="font-bold text-sm text-[#A67C52]">เพิ่มเมนูใหม่</h3>
                  <input type="text" placeholder="ชื่อเมนู" className="w-full p-3 rounded-xl text-sm outline-none" value={newMenu.name} onChange={e => setNewMenu({...newMenu, name: e.target.value})} />
                  <div className="flex gap-2">
                    <input type="number" placeholder="ราคา" className="w-1/2 p-3 rounded-xl text-sm outline-none" value={newMenu.price} onChange={e => setNewMenu({...newMenu, price: e.target.value})} />
                    <select className="w-1/2 p-3 rounded-xl text-sm outline-none" value={newMenu.category} onChange={e => setNewMenu({...newMenu, category: e.target.value})}>
                      {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                  <label className="cursor-pointer bg-white border p-3 rounded-xl text-xs font-bold block">
                    {newMenu.image ? 'เปลี่ยนรูปเมนู' : 'อัปโหลดรูปภาพเมนู'}
                    <input type="file" accept="image/*" className="hidden" onChange={e => {
                      const fr = new FileReader(); fr.onload = (ev) => setNewMenu({...newMenu, image: ev.target.result}); fr.readAsDataURL(e.target.files[0]);
                    }} />
                  </label>
                  {newMenu.image && <img src={newMenu.image} className="w-20 h-20 mx-auto rounded-xl object-cover" />}
                  <button onClick={handleAddMenu} className="w-full bg-[#A67C52] text-white py-3 rounded-xl font-bold text-sm">บันทึกเมนูใหม่</button>
                </div>
                <div className="space-y-3">
                   {menuItems.map(item => (
                     <div key={item.id} className="flex justify-between items-center bg-white p-3 rounded-2xl border">
                       <div className="flex items-center gap-3"><img src={item.image} className="w-12 h-12 rounded-xl object-cover" /><div><p className="font-bold text-sm">{item.name}</p><p className="text-xs text-gray-400">฿{item.price}</p></div></div>
                       <button onClick={() => handleDeleteMenu(item.id)} className="p-3 text-red-400"><Trash2 size={18}/></button>
                     </div>
                   ))}
                </div>
              </div>
            ...
            {adminTab === 'settings' && (
              <div className="bg-gray-50 p-5 rounded-3xl border-2 border-dashed border-gray-200 space-y-4 text-center">
                ...
                <input type="text" placeholder="หมายเลขพร้อมเพย์" className="w-full p-3 rounded-xl text-sm outline-none" value={editPromptPay} onChange={e => setEditPromptPay(e.target.value)} />
                <label className="cursor-pointer bg-white border p-3 rounded-xl text-xs font-bold block">
                  {editQrCodeImage ? 'เปลี่ยนรูป QR Code' : 'อัปโหลดรูป QR Code ร้าน'}
                  <input type="file" accept="image/*" className="hidden" onChange={e => {
                    const fr = new FileReader(); fr.onload = (ev) => setEditQrCodeImage(ev.target.result); fr.readAsDataURL(e.target.files[0]);
                  }} />
                </label>
                {editQrCodeImage && <img src={editQrCodeImage} className="w-20 h-20 mx-auto rounded-xl object-contain" />}
                <button onClick={async () => {
                  await setDoc(doc(db, 'settings', 'store'), { promptPayNo: editPromptPay, qrCodeImage: editQrCodeImage }, { merge: true });
                  alert('บันทึกสำเร็จ! 🐮');
                }} className="w-full bg-[#3D2C1E] text-white py-3 rounded-xl font-bold text-sm">บันทึกการตั้งค่า</button>
              </div>
            )}
          </div>
        )}
      ...
      {selectedSlip && (
        <div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center p-4 animate-in fade-in" onClick={() => setSelectedSlip(null)}>
          <button className="absolute top-10 right-10 text-white p-2 bg-white/10 rounded-full"><X size={30}/></button>
          <img src={selectedSlip} className="max-w-full max-h-[85vh] rounded-xl shadow-2xl animate-in zoom-in duration-300" />
        </div>
      )}
      ...
      {showAdminModal && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center backdrop-blur-sm p-4">
          <div className="bg-white p-8 rounded-3xl w-full max-w-sm animate-in zoom-in duration-200">
            <h3 className="font-bold text-xl mb-4">เข้าสู่ระบบแอดมิน</h3>
            <input type="password" value={adminPassword} onChange={e => setAdminPassword(e.target.value)} className="w-full bg-gray-50 border p-4 rounded-2xl mb-6 text-center text-2xl outline-none" placeholder="••••••" />
      ...
      {optionModalItem && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-end justify-center backdrop-blur-sm p-4">
          <div className="bg-white rounded-[3rem] w-full max-w-md p-8 space-y-8 animate-in slide-in-from-bottom-full duration-300">
            <div className="flex justify-between items-center"><h3 className="text-2xl font-serif font-bold">{optionModalItem.name}</h3><button onClick={() => setOptionModalItem(null)} className="p-3 bg-gray-50 rounded-2xl text-gray-400"><X/></button></div>
            <div className="space-y-6">
              <div><label className="text-sm font-bold block mb-4 text-gray-400">ระดับความหวาน</label>
                <div className="grid grid-cols-5 gap-2">{SWEETNESS.map(l => (
                    <button key={l} onClick={() => setTempOptions({...tempOptions, sweetness: l})} className={`py-3 rounded-2xl text-[10px] font-bold border ${tempOptions.sweetness === l ? 'bg-[#3D2C1E] text-white' : 'bg-white text-gray-300'}`}>{l}</button>
                ))}</div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <button onClick={() => setTempOptions({...tempOptions, isBlended: false})} className={`py-6 rounded-3xl border-2 font-bold flex flex-col items-center gap-3 ${!tempOptions.isBlended ? 'border-[#A67C52] bg-[#F5EEDC]/40 text-[#3D2C1E]' : 'text-gray-300'}`}><Coffee size={28}/><span className="text-xs">เมนูเย็น</span></button>
                 <button onClick={() => setTempOptions({...tempOptions, isBlended: true})} className={`py-6 rounded-3xl border-2 font-bold flex flex-col items-center gap-3 ${tempOptions.isBlended ? 'border-[#A67C52] bg-[#F5EEDC]/40 text-[#3D2C1E]' : 'text-gray-300'}`}><Zap size={28}/><span className="text-xs">เมนูปั่น (+฿{optionModalItem.blendPrice || 5})</span></button>
              </div>
            </div>
            <button onClick={() => {
              ...
              }} className="w-full py-5 bg-[#3D2C1E] text-white rounded-[2.5rem] font-bold text-lg"><Plus size={20} className="inline mr-2"/> เพิ่มลงตะกร้า • ฿{optionModalItem.price + (tempOptions.isBlended ? (optionModalItem.blendPrice || 5) : 0)}</button>
          </div>
        </div>
      )}
    </div>
